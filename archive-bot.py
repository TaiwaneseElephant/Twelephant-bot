...
TIME_STAMP_PATTERN = re.compile(r"\d{4}年\d{1,2}月\d{1,2}日 \([一二三四五六日]\) \d{2}:\d{2} \(UTC\)")
...
def archive_page(page_name:str, site, archive_page_name:str = "%(page)s/存檔%(counter)d", archive_time:list = ["old", 86400], counter:int = 1, minthreadsleft:int = 5, minthreadstoarchive:int = 2, \
    ...
    for i in range(threads_num):
        ...
        if (signature_timestamp := find_signature_timestamp(content)):
            title = sections.sections[i].heading
                    ...
                    if rule.match(title):
                        if custom_time_type == "old":
                            ...
                            if custom_standard[0] == "y":
                               ...
                               for j in signature_timestamp:
                                    time_then = timestripper.timestripper(j).timetuple()
                                    if ((time.gmtime().tm_year - time_then.tm_year) * 365 + (time.gmtime().tm_yday - time_then.tm_yday)) < (custom_standard[1] * 7):
                                        fail = True
                                        break
                                    if calendar.timegm(time_then) > last_timestamp:
                                        last_time = time_then
            ...
            if not custom_rules_used:
                ...
                if time_type == "old":
                    ...
                    if archive_standard[0] == "y":
                       ...
                       for j in signature_timestamp:
                            time_then = timestripper.timestripper(j).timetuple()
                            if ((time.gmtime().tm_year - time_then.tm_year) * 365 + (time.gmtime().tm_yday - time_then.tm_yday)) < (archive_standard[1] * 7):
                                fail = True
                                break
                            if calendar.timegm(time_then) > last_timestamp:
                                last_time = time_then
    ...
    if del_list != set() and len(del_list) >= minthreadstoarchive:
        if counter_used:
            ...
            for i, j in archive_list.items():
                ...
                archive(archive_page_name = achive_name, site = site, archive_list = j, sections = sections, talk_page_name = page_name, \
                        header = archiveheader, counter_used = False)
            del_archived(site, talk_page, del_list, archive_list[(0, 0)])
        else:
